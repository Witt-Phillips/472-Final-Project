...
class BerryProb:
    ...
    def init_effect(self, pi, effect, pos):
        #Set
        setattr(pi, pos, effect)
        comp_str = "first_c" if pos == "init_1" else "second_c"
        setattr(pi, comp_str, self.find_c(effect))
        complement = getattr(pi, comp_str) #None if complement does not exist
        result = 1 if pos == "init_1" else 0

        #Update all to reflect result

        if complement is not None:
            # Pulling from complement to update
            n_k = self.n_k_from_comp(complement, pi)
            c_obj = self.probList[getattr(pi, complement)]
            pi.prob = bayesian_update(pi.prob, likelihood(n_k[0] + result, n_k[1], p))

            # Updating Complement _UPDATE FOR RESULT
            same_idx = True if c_obj.first == pi.first or c_obj.second == pi.second else False
            k = 1 if same_idx is False else 0
            c_obj.prob = bayesian_update(c_obj.prob, likelihood(k, 1, p))
            pi.dist.append(result)
